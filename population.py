...
class PopulationTree(AbstractTree):
    ...
    def get_separator(self: PopulationTree) -> str:
...
def _load_data() -> List[PopulationTree]:
    ...
    for region in regions:
        for country in regions[region]:
            if country in country_populations.keys():
                lst.append(PopulationTree(
                    False, country, None, country_populations[country]))
        ans.append(PopulationTree(False, region, lst))
        lst = []
    return ans
...
def _get_population_data() -> Dict[str, int]:
    ...
    num = len(population_data)
    for i in range(num):
        key = population_data[i]["country"]['value']
        val = population_data[i]["value"]
        if key and val:
...
def _get_region_data() -> Dict[str, List[str]]:
    ...
    for data in country_data:
        ...
        if val != "Aggregates" and "name" in data.keys():
            if val not in regions.keys():
                regions[val] = [data["name"]]
            else:
                regions[val].append(data["name"])
    return regions
